...
func (t *Handler) newTaskExecutionContext(ctx context.Context, nCtx handler.NodeExecutionContext, pluginID string) (*taskExecutionContext, error) {
	...
	outputSandbox, err := ioutils.NewRandomPrefixShardedOutputSandbox(ctx, t.sharder, nCtx.OutputDataSandboxBasePath(), nCtx.NodeStatus().GetOutputDir(), nCtx.DataStore())
	if err != nil {
